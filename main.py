...
# пересечение со сферой. Квадратное уравнение через дискриминант. Пересечение тогда, когда дискриминант больше 0
def sphere_intersect(center, radius, ray_origin, ray_direction):
    b = 2 * np.dot(ray_direction, ray_origin - center)
    c = np.linalg.norm(ray_origin - center) ** 2 - radius ** 2
    discrim = b ** 2 - 4 * c
    if discrim > 0:
        t1 = (-b + np.sqrt(discrim)) / 2
        t2 = (-b - np.sqrt(discrim)) / 2
        if t1 > 0 and t2 > 0:
            return min(t1, t2) #ближайште точки пересечения
    return None


# ближайший пересекаемый объект. перебираем все сферы, ищем пересечения и сохраняем ближайшую сферу.
...
for i, y in enumerate(np.linspace(screen[1], screen[3], height)):
    # вычисление исходной точки и направления, которые вместе определяют луч.
    for j, x in enumerate(np.linspace(screen[0], screen[2], width)):
        ...
        for k in range(max_depth):
            ...
            # освещенность точки моделью Блинна-Фонга
            if is_shadowed:
                break
